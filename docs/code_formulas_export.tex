...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={LLC 设计工具 — 代码公式导出（LaTeX 版）},
    pdfauthor={LLC Design Tool}
}
...
\title{\Huge LLC 设计工具 — 代码公式导出（LaTeX 版）}
\author{提取自 LLC 设计工具源代码}
\date{\today}
...
\section*{LLC 设计工具 — 代码公式导出}

本文档全面提取自当前 LLC 设计工具源代码（\texttt{src/}）中的算术/设计计算表达式及显示的数学公式。

\begin{itemize}
\item \textbf{代码计算文件}（\texttt{Designer.tsx}、\texttt{Curves.tsx}、\texttt{Report.tsx}、\texttt{CompensationSection.tsx}、\texttt{DesignCompare.tsx}）：提取为 Markdown/LaTeX 公式，并附简要说明。
\item \textbf{推导页面}（\texttt{Derivations.tsx}、\texttt{Operation.tsx}、\texttt{Fundamentals.tsx}）：提取自 \texttt{MathBlock latex="..."} / \texttt{MathBlock} 的 KaTeX 字符串。
\item \texttt{Qmax2}/\texttt{16}、\texttt{Coss} 损耗 \texttt{2/3}、体二极管 \texttt{0.7} 的工程注释插入在相关公式出现的位置。
\end{itemize}

\noindent\rule{\textwidth}{0.4pt}

\subsection*{1. \texttt{src/pages/Designer.tsx} — 主设计计算与损耗分析}

\subsubsection*{1.1 E 系列标准值辅助函数}
...
返回 \texttt{E12}/\texttt{E24} 中最接近的尾数乘以 \(10^{\text{exponent}}\)。

\subsubsection*{1.2 LLC 电压增益（FHA）}
...
\textbf{注：} 峰值增益通过对 \(f_n = 0.3 \sim 1.0\) 进行数值搜索得到（\texttt{peakGain}）。

\subsubsection*{1.3 谐振点处的 ZVS 相位}
...
\subsubsection*{1.4 主设计计算流程（\texttt{handleCalculate}）}
...
\paragraph{Qmax1 — 峰值增益约束}

通过数值二分法（\texttt{findQmax1}）求解 \(Q_{max1}\)，使得峰值增益等于 \(G_{max}\)。

\paragraph{Qmax2 — ZVS 死区时间约束}
...
\subsubsection*{1.5 损耗分析（\texttt{calculateLosses}）}
...
（单位：\texttt{mosfetRdsOn} 以 mΩ 输入，内部转换为 Ω。）
...
\subsection*{2. \texttt{src/pages/Curves.tsx} — 增益与阻抗特性曲线}

\subsubsection*{2.1 LLC 增益（归一化）}
...
\subsubsection*{2.2 输入阻抗（相对于 \(Z_r\) 归一化）}
...
\subsection*{3. \texttt{src/pages/Report.tsx} — 设计报告生成}

\subsubsection*{3.1 报告表格中使用的导出参数}

\[
R_{ac} = \frac{8 n^{2} V_{out}^{2}}{\pi^{2} P_{out}}
\]

\subsubsection*{3.2 所需增益}
...
\subsection*{4. \texttt{src/components/CompensationSection.tsx} — 环路补偿设计}

\subsubsection*{4.1 E 系列 / 数值格式化}

与 Designer 中相同的 \texttt{nearestE} 辅助函数。

\subsubsection*{4.2 功率级极点}
...
\subsubsection*{4.3 被控对象传递函数模型}

\textbf{积分器：}

\[
G_p(s) = \frac{K_p}{s},\qquad |G_p| = \frac{K_p}{\omega},\qquad \angle G_p = -90^{\circ}
\]
...
\[
\angle G_p = -90^{\circ} - \tan^{-1}\left(\frac{\omega}{\omega_{p,out}}\right)
\]
...
\[
\angle G_p = -90^{\circ} + \tan^{-1}\left(\frac{\omega}{\omega_{p,zero}}\right) - \tan^{-1}\left(\frac{\omega}{\omega_{p,out}}\right)
\]

\subsubsection*{4.4 补偿器增益/相位}

Type II / Type III 传递函数的幅值/相位在每个频率点进行数值计算。

\subsubsection*{4.5 穿越频率处所需补偿器增益}

\[
G_{c,needed} = \frac{1}{G_{p,linear}(f_c)}
\]

\subsubsection*{4.6 K 因子设计}

所需补偿器相位：

\[
\varphi_{c,req} = PM_{target} - 180^{\circ} - \varphi_{plant}(f_c)
\]

目标相位提升：

\[
\text{boost}_{target} = \varphi_{c,req} + 180^{\circ}\quad (\text{限制在实用范围内})
\]
...
\[
K = \tan\left(\frac{(\text{boost}_{target} + 180^{\circ})\pi}{720}\right),\qquad
f_{z1}=f_{z2}=\frac{f_c}{K},\qquad
f_{p1}=f_{p2}=K f_c
\]

\subsubsection*{4.7 元件计算}
...
\subsubsection*{4.8 校验}

\[
PM_{actual} = 180^{\circ} + \varphi_{plant}(f_c) + \varphi_{comp}(f_c)
\]
...
\subsection*{5. \texttt{src/components/DesignCompare.tsx} — A/B 设计方案对比}

\subsubsection*{5.1 增益曲线对比}

对每个保存的设计评估相同的 FHA \texttt{calcGain} 公式。

\subsubsection*{5.2 设计评分}
...
\subsection*{6. \texttt{src/pages/Derivations.tsx} — 公式推导（KaTeX 行间公式）}

\subsubsection*{6.1 第 1 节 — 基本拓扑与工作原理}
...
\subsubsection*{6.2 第 2 节 — 稳态增益与 FHA 推导}
...
\subsubsection*{6.3 第 3 节 — 谐振频率与特征参数}
...
\subsubsection*{6.4 第 4 节 — 峰值增益与边界条件}
...
\subsubsection*{6.5 第 5 节 — 输入阻抗与 ZVS 条件}
...
\subsubsection*{6.6 第 6 节 — 功率器件应力计算}
...
\subsubsection*{6.7 第 7 节 — 功率器件损耗模型}
...
\subsubsection*{6.8 第 8 节 — 谐振腔与变压器设计}
...
\subsection*{7. \texttt{src/pages/Operation.tsx} — LLC 工作原理（KaTeX 行间公式）}

\subsubsection*{7.1 模式边界谐振频率}
...
\subsection*{8. \texttt{src/pages/Fundamentals.tsx} — 谐振基础（KaTeX 行间公式）}

\subsubsection*{8.1 串联 LC 谐振}
...
\subsubsection*{8.2 品质因数}

\[
Q = \frac{Z_r}{R_{ac}} = \frac{\sqrt{L_r/C_r}}{R_{ac}}
\]

\subsubsection*{8.3 带宽}

\[
BW = \frac{f_r}{Q} = f_2 - f_1
\]

\subsubsection*{8.4 FHA 归一化增益}

\[
M = \frac{n \cdot V_{out}}{V_{in}} \quad (\text{全桥})
\]

\[
M = \frac{2n \cdot V_{out}}{V_{in}} \quad (\text{半桥})
\]

\subsubsection*{8.5 关键参数定义}
...
\begin{itemize}
\item \textbf{写入文件：} \texttt{docs/code\_formulas\_export.md}
\item \textbf{公式来源：}
\begin{itemize}
\item \texttt{src/pages/Designer.tsx}
\item \texttt{src/pages/Curves.tsx}
\item \texttt{src/pages/Report.tsx}
\item \texttt{src/components/CompensationSection.tsx}
\item \texttt{src/components/DesignCompare.tsx}
\item \texttt{src/pages/Derivations.tsx}（KaTeX \texttt{MathBlock}）
\item \texttt{src/pages/Operation.tsx}（KaTeX \texttt{MathBlock}）
\item \texttt{src/pages/Fundamentals.tsx}（KaTeX \texttt{MathBlock}）
\end{itemize}
\item \textbf{包含的工程注释：} \texttt{Qmax2}/\texttt{16}、\texttt{Coss} 损耗 \texttt{2/3}、体二极管 \texttt{0.7}。
\end{itemize}
